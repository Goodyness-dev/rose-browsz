...
export default function EditorialHome({ onOpenWizard, onNavigate, onScrollToSection, darkMode, onToggleDarkMode }) {
  ...
  useEffect(() => {
    if (!menu) return;
    const close = e => { if (e.key === 'Escape') { setMenu(false); root.current?.querySelector('.rb-menu-toggle')?.focus(); } };
    window.addEventListener('keydown', close);
    return () => window.removeEventListener('keydown', close);
  }, [menu]);
  const jump = id => { setMenu(false); onScrollToSection(id); };
  return <div ref={root} className="rb-site">
    <a className="rb-skip" href="#rb-main">Skip to content</a>
    <header className="rb-nav">
      <a className="rb-logo" href="#" aria-label="Rose Browsz home">rose browsz<span>BEAUTY STUDIO & ACADEMY</span></a>
      <nav className="rb-desktop-nav" aria-label="Main navigation">
        <button onClick={() => jump('services')}>The treatments</button>
        <button onClick={() => jump('artist')}>Meet Jess</button>
        <button onClick={() => jump('location')}>Visit us</button>
      </nav>
      <div className="rb-nav-actions">
        ...
        <a className="rb-button rb-nav-book" href={business.bookingUrl}>Book a visit <span aria-hidden="true">↗</span></a>
        <button className="rb-menu-toggle" aria-expanded={menu} aria-controls="rb-mobile-menu" onClick={() => setMenu(!menu)}>{menu ? 'Close' : 'Menu'}</button>
      </div>
      {menu && <nav id="rb-mobile-menu" className="rb-mobile-nav" aria-label="Mobile navigation">
        <button onClick={() => jump('services')}>The treatments</button><button onClick={() => jump('artist')}>Meet Jess</button><button onClick={() => jump('location')}>Visit us</button><a href={business.bookingUrl}>Book a visit ↗</a>
      </nav>}
    </header>
    <div id="rb-main">
      <section className="rb-hero">
        <div className="rb-hero-copy">
          ...
          <div className="rb-hero-actions"><a className="rb-button" href={business.bookingUrl}>Find your signature look <span aria-hidden="true">↗</span></a><button className="rb-text-link" onClick={() => onOpenWizard()}>Let's talk first</button></div>
      ...
      <section id="services" className="rb-services">
        ...
        <div className="rb-stack">{services.map((s, i) => <article className="rb-card" key={s.label} style={{ '--card-index': i }}>
          <div className="rb-card-inner card-thick">
            <div className="rb-card-copy"><p className="rb-eyebrow">{s.label}</p><h3>{s.title.split('\n').map((line, index) => <React.Fragment key={line}>{index > 0 && <br/>}{index ? <em>{line}</em> : line}</React.Fragment>)}</h3><p>{s.text}</p><small>{s.detail}</small><button className="rb-card-link" onClick={() => i === 2 ? onOpenWizard() : onNavigate('services')}>{s.action}<span aria-hidden="true">↗</span></button></div>
      ...
      <section id="location" className="rb-visit rb-reveal"><div><p className="rb-eyebrow">YOUR LITTLE MOMENT OF ME-TIME</p><h2>See you<br/><em>in the studio.</em></h2><a className="rb-button" href={business.bookingUrl}>Make time for you <span aria-hidden="true">↗</span></a></div><div className="rb-visit-details"><p>ROSE BROWSZ & BEAUTY ACADEMY</p><address>973 Pleasant Grove Blvd, Suite #130<br/>Roseville, CA 95678</address><p>Tuesday–Saturday / 9am–4pm<br/>Sunday & Monday / Closed</p><a className="rb-text-link" href={business.googleMapsLink} target="_blank" rel="noreferrer">Find the studio ↗</a><a href="tel:9166262076">{business.phone}</a></div></section>
    </div>
    <footer className="rb-footer"><div className="rb-footer-top"><a className="rb-logo" href="#">rose browsz<span>BEAUTY STUDIO & ACADEMY</span></a><div><a href={business.instagram} target="_blank" rel="noreferrer">Instagram ↗</a><a href={'mailto:' + business.email}>Say hello ↗</a><button onClick={() => onNavigate('admin')}>Studio login</button></div></div><div className="rb-footer-bottom"><span>© {new Date().getFullYear()} Rose Browsz LLC</span><span>BEAUTY, IN YOUR OWN WAY.</span><button onClick={() => window.__lenis ? window.__lenis.scrollTo(0) : window.scrollTo({ top: 0, behavior: 'smooth' })}>Back to top ↑</button></div></footer>
  </div>;
}
